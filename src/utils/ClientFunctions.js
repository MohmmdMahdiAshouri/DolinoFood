...
export const isCart = (cart, food) => {
    if(cart){
        if(!food) return false
        const isFood = cart.find(item => String(item._id) === String(food._id))
        return isFood
    }
    return false
}
...
export const converDate = (ms) => {
    ...
    const format = date.format("jYYYY/jMM/jDD")

    const year = date.jYear()
    const month = date.jMonth() + 1
